...
private static final Scanner sc = new Scanner(System.in); // Shared Scanner object
...
public static void libraryLateFees() {
    ...
    switch (type) {
        case "regular" -> fee = days * 0.50;
        case "fiction" -> fee = days * 0.75;
        case "reference" -> fee = 0;
        default -> {
    ...
    System.out.printf("Total Late Fee: $%.2f%n", fee);
}
...
public static void shopDiscount() {
    ...
    System.out.printf("Total Price after Discount: $%.2f%n", price);
}
...
public static void mobilePlan() {
    ...
    if (minutes <= 100 && messages <= 100) {
        ...
        cost = 40 + ((minutes > 100) ? (minutes - 100) * 0.10 : 0) + ((messages > 100) ? (messages - 100) * 0.05 : 0);
    } else {
        cost = 60;
    }

    System.out.printf("Total Mobile Plan Cost: $%.2f%n", cost);
}
...
public static void shippingCost() {
    ...
    System.out.print("Enter shipping distance (miles): ");
    int distance = sc.nextInt();
    int cost = (weight <= 10 && distance <= 100) ? 5 :
            (weight <= 20 || distance <= 500) ? 10 : 20;
